...
from resumenes import mostrar_resumenes
from videos import get_videos
from examen_fifa import (preguntas_agente_fifa, preguntas_estatuto_transferencia,
                         preguntas_codigo_disciplinario, preguntas_estatutos_fifa, preguntas_salvaguardia)
from esquemas import (esquema_formacion, confederacion_afc, confederacion_caf,
                      confederacion_concacaf, confederacion_conmebol, confederacion_ofc, confederacion_uefa)
...
def mostrar_inicio():
    ...
    - **📝 Exámenes**: Realiza exámenes relacionados con la temática FIFA, tipo test.
    ...
    - **:heavy_division_sign: Calculadora**: Calculadora para realizar cálculos sobre comisiones entre los distintos participantes.
    - **:dart: Exámenes Oficiales FIFA**: Preguntas oficiales sobre exámenes de la FIFA ya realizados.
    """)

# Exámenes tipo test 1 respuesta
def mostrar_examen():
...
# Calculadora de pagos variables
...
# Función Ventanas de mercado calendarios
...
# Visualización
def main():
    tabs = ["Inicio", "Examen oficial FIFA", "Exámenes para practicar", "Resúmenes", "Formación", "Temario", "Videos", "Calculadora", "Podcast"]
    tab_select = st.sidebar.selectbox("Selecciona una sección", tabs, index=0)

    if tab_select == "Inicio":
        mostrar_inicio()
    elif tab_select == "Exámenes para practicar":
        mostrar_examen()
        if st.button("Realizar otro examen"):
            st.session_state['sesion'] = iniciar_sesion()
            st.experimental_rerun()
    elif tab_select == "Resúmenes":
